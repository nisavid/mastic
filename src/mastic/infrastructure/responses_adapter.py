    ...
    def __post_init__(self) -> None:
        ...
        if parsed.scheme not in {"http", "https"} or not parsed.hostname:
            raise ConfigurationError(
                "MASTIC_RESPONSES_ADAPTER_UPSTREAM must be an explicit HTTP(S) URL"
            )
        if parsed.username or parsed.password or parsed.query or parsed.fragment:
            raise ConfigurationError(
                "MASTIC_RESPONSES_ADAPTER_UPSTREAM must not contain credentials, a query, or a fragment"
            )
        if not _is_loopback_host(self.listen_host):
            raise ConfigurationError(
                "MASTIC_RESPONSES_ADAPTER_HOST must resolve only to loopback addresses"
            )
        if not (0 <= self.listen_port <= 65535):
            raise ConfigurationError(
                "MASTIC_RESPONSES_ADAPTER_PORT must be between 0 and 65535"
            )
        if len(self.inbound_token) < 32:
            raise ConfigurationError(
                "MASTIC_RESPONSES_ADAPTER_INBOUND_TOKEN must be at least 32 characters"
            )
        if not _valid_bearer_value(self.inbound_token):
            raise ConfigurationError(
                "MASTIC_RESPONSES_ADAPTER_INBOUND_TOKEN contains invalid characters"
            )
        if self.upstream_token is not None and not _valid_bearer_value(
            self.upstream_token
        ):
            raise ConfigurationError(
                "MASTIC_RESPONSES_ADAPTER_UPSTREAM_TOKEN contains invalid characters"
            )
        ...
        if self.max_body_bytes <= 0:
            raise ConfigurationError(
                "MASTIC_RESPONSES_ADAPTER_MAX_BODY_BYTES must be positive"
            )
        ...
        if (
            not math.isfinite(self.sse_heartbeat_seconds)
            or self.sse_heartbeat_seconds <= 0
        ):
            raise ConfigurationError(
                "MASTIC_RESPONSES_ADAPTER_SSE_HEARTBEAT must be positive"
            )
        if self.adapter not in {"identity", "codex-namespace"}:
            raise ConfigurationError(
                "MASTIC_RESPONSES_ADAPTER_ADAPTER must be identity or codex-namespace"
            )
        if self.namespace_map_capacity <= 0:
            raise ConfigurationError(
                "MASTIC_RESPONSES_ADAPTER_NAMESPACE_MAP_CAPACITY must be positive"
            )

    @classmethod
    def from_env(
        cls, env: dict[str, str] | os._Environ[str] = os.environ
    ) -> "ProxyConfig":
        upstream = env.get("MASTIC_RESPONSES_ADAPTER_UPSTREAM")
        if not upstream:
            raise ConfigurationError("MASTIC_RESPONSES_ADAPTER_UPSTREAM is required")
        inbound_token = env.get("MASTIC_RESPONSES_ADAPTER_INBOUND_TOKEN")
        if not inbound_token:
            raise ConfigurationError(
                "MASTIC_RESPONSES_ADAPTER_INBOUND_TOKEN is required"
            )
        return cls(
            upstream_url=upstream,
            inbound_token=inbound_token,
            upstream_token=env.get("MASTIC_RESPONSES_ADAPTER_UPSTREAM_TOKEN") or None,
            listen_host=env.get("MASTIC_RESPONSES_ADAPTER_HOST", "127.0.0.1"),
            listen_port=int(env.get("MASTIC_RESPONSES_ADAPTER_PORT", "18999")),
            max_body_bytes=int(
                env.get("MASTIC_RESPONSES_ADAPTER_MAX_BODY_BYTES", str(8 * 1024 * 1024))
            ),
            upstream_timeout_seconds=float(
                env.get("MASTIC_RESPONSES_ADAPTER_UPSTREAM_TIMEOUT", "30")
            ),
            inbound_timeout_seconds=float(
                env.get("MASTIC_RESPONSES_ADAPTER_INBOUND_TIMEOUT", "30")
            ),
            sse_heartbeat_seconds=float(
                env.get("MASTIC_RESPONSES_ADAPTER_SSE_HEARTBEAT", "15")
            ),
            adapter=env.get("MASTIC_RESPONSES_ADAPTER_ADAPTER", "identity"),
            namespace_map_capacity=int(
                env.get("MASTIC_RESPONSES_ADAPTER_NAMESPACE_MAP_CAPACITY", "256")
            ),
            debug=_env_bool(env.get("MASTIC_RESPONSES_ADAPTER_DEBUG")),
        )
...
def transform_response(data: Any, reconstruction: dict[str, tuple[str, str]]) -> Any:
...
def _safe_log(message: str) -> None:
    sys.stderr.write(f"[responses-adapter] {message}\n")
    sys.stderr.flush()
...
class _SSELineReader:
    ...
    def __init__(
        ...
        self._thread = threading.Thread(
            target=self._read,
            name="responses-adapter-sse-reader",
            daemon=True,
        )
        self._thread.start()

    def next_item(self, timeout: float) -> tuple[str, bytes | Exception] | None:
        try:
...
def _handler_for(config: ProxyConfig, namespace_state: _NamespaceState):
    class ProxyHandler(http.server.BaseHTTPRequestHandler):
        ...
        server_version = "responses-adapter"
        sys_version = ""
        ...
        def _send_route_error(self) -> None:
            allowed = sorted(
                method for method, path in ALLOWED_ROUTES if path == self.path
            )
            ...
            self._send_json(
                ...
                {
                    "error": {
                        "message": "method not allowed" if allowed else "not found"
                    }
                },
                headers,
            )
        ...
        def _validate_body_framing(self) -> int | None:
            if self.headers.get_all("Transfer-Encoding", failobj=[]):
                self._send_json(
                    400, {"error": {"message": "transfer encoding is not supported"}}
                )
                return None
        ...
        def _read_json_body(self) -> dict[str, Any] | None:
            ...
            if not isinstance(parsed, dict):
                self._send_json(
                    400, {"error": {"message": "JSON body must be an object"}}
                )
                return None
            return parsed
            ...
            try:
                ...
                if not self._response_started and not self.wfile.closed:
                    if self.server.is_closing():
                        ...
                        self._send_json(
                            502, {"error": {"message": "upstream unavailable"}}
                        )
            finally:
            ...
            if "json" in content_type.lower() and raw:
                try:
                    ...
                    if transformed != original:
                        raw = json.dumps(transformed, separators=(",", ":")).encode(
                            "utf-8"
                        )
                except (json.JSONDecodeError, UnicodeDecodeError):
                    pass
        ...
        def _stream_sse(
            ...
            try:
                while True:
                    ...
                    if item is None:
                        if not frame and not terminal_observed:
                            self.wfile.write(b": responses-adapter keep-alive\n\n")
                            self.wfile.flush()
                        continue
                    kind, value = item
                    if kind == "error":
                ...
                if config.debug and not terminal_logged:
                    _safe_log(
                        f"SSE terminal_completed={str(terminal_observed).lower()}"
                    )
