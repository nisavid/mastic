"""Application-owned setup and removal coordination.

Resolution stays side-effect free. An exact reviewed Plan crosses one typed
nested-operation contract only after confirmation. Its separate capability
ports preserve physical ownership boundaries.
"""
...
class SetupNestedOperation:
    """Exact Completion identity shared by a subordinate Plan operation."""

    step_fingerprint: str

    def __post_init__(self) -> None:
        if len(self.step_fingerprint) != 64 or any(
            character not in "0123456789abcdef" for character in self.step_fingerprint
        ):
            raise ValueError("setup step fingerprint must be lowercase sha256")


@dataclass(frozen=True, slots=True, kw_only=True)
class ActivateSupervisor(SetupNestedOperation):
    pass


@dataclass(frozen=True, slots=True, kw_only=True)
class InstallRuntime(SetupNestedOperation):
    runtime: str
...
@dataclass(frozen=True, slots=True, kw_only=True)
class InstallModel(SetupNestedOperation):
    repository: str
    revision: str
    alias: str | None
    offline: bool
...
@dataclass(frozen=True, slots=True, kw_only=True)
class TrustModel(SetupNestedOperation):
    resource: str
    runtime: str
    revision: str
...
@dataclass(frozen=True, slots=True, kw_only=True)
class ConfigureService(SetupNestedOperation):
    service: str
    model_alias: str
    runtime: str
    route: str
    ...
    def __post_init__(self) -> None:
        super(ConfigureService, self).__post_init__()
        object.__setattr__(self, "options", MappingProxyType(dict(self.options)))


@dataclass(frozen=True, slots=True, kw_only=True)
class ConfigureGateway(SetupNestedOperation):
    host: str
    port: int


@dataclass(frozen=True, slots=True, kw_only=True)
class InstallApplications(SetupNestedOperation):
    application_targets: tuple[str, ...]
    preserve_outdated_codex: bool
    offline: bool


@dataclass(frozen=True, slots=True, kw_only=True)
class ConfigureApplicationTarget(SetupNestedOperation):
    ...
    def __post_init__(self) -> None:
        super(ConfigureApplicationTarget, self).__post_init__()
        object.__setattr__(self, "options", MappingProxyType(dict(self.options)))


@dataclass(frozen=True, slots=True, kw_only=True)
class StartService(SetupNestedOperation):
    resource: str


@dataclass(frozen=True, slots=True, kw_only=True)
class TestApplicationTarget(SetupNestedOperation):
    application_target: str
    profile: str


@dataclass(frozen=True, slots=True, kw_only=True)
class VerifyGatewayRequest(SetupNestedOperation):
    endpoint: str
    model: str
    request: str
...
@dataclass(frozen=True, slots=True, kw_only=True)
class DrainService(SetupNestedOperation):
    resource: str


@dataclass(frozen=True, slots=True, kw_only=True)
class StopService(SetupNestedOperation):
    resource: str


@dataclass(frozen=True, slots=True, kw_only=True)
class UnregisterSupervisor(SetupNestedOperation):
    pass


@dataclass(frozen=True, slots=True, kw_only=True)
class RemoveApplicationTarget(SetupNestedOperation):
    application_target: str


@dataclass(frozen=True, slots=True, kw_only=True)
class RemoveApplications(SetupNestedOperation):
    applications: tuple[str, ...]


@dataclass(frozen=True, slots=True, kw_only=True)
class RemoveState(SetupNestedOperation):
    paths: tuple[str, ...]


class SetupDesiredState(Protocol):
    """Apply exact Plan declarations through desired-state storage."""

    def configure_gateway(
        self, operation: ConfigureGateway
    ) -> Mapping[str, object]: ...

    def trust_model(self, operation: TrustModel) -> Mapping[str, object]: ...

    def configure_service(
        self, operation: ConfigureService
    ) -> Mapping[str, object]: ...


class SetupRuntimeSupply(Protocol):
    """Materialize one exact Runtime Installation."""

    def install_runtime(self, operation: InstallRuntime) -> Mapping[str, object]: ...


class SetupModelSupply(Protocol):
    """Materialize one exact Model Installation."""

    def install_model(self, operation: InstallModel) -> Mapping[str, object]: ...


class SetupExternalApplicationLifecycle(Protocol):
    """Coordinate exact External Application Installation lifecycle work."""
...
class SetupApplicationConfiguration(Protocol):
    """Configure and remove Application Configuration Targets."""
...
class SetupNativeCanaries(Protocol):
    """Exercise application-native verification contracts."""
...
class SetupServiceLifecycle(Protocol):
    """Control Supervisor and Inference Service lifecycle units."""
...
class SetupGatewayVerification(Protocol):
    """Verify the first request through the selected Gateway route."""

    def verify_gateway(
        self, operation: VerifyGatewayRequest
    ) -> Mapping[str, object]: ...


class SetupProductStateRemoval(Protocol):
    """Delete only the product-owned state selected by reviewed product removal."""

    def remove_state(self, operation: RemoveState) -> Mapping[str, object]: ...


@dataclass(frozen=True, slots=True)
class SetupNestedOperations:
    """One typed nested-operation contract composed of separate capability ports."""

    desired_state: SetupDesiredState
    runtime_supply: SetupRuntimeSupply
    model_supply: SetupModelSupply
    application_lifecycle: SetupExternalApplicationLifecycle
    application_configuration: SetupApplicationConfiguration
    native_canaries: SetupNativeCanaries
    service_lifecycle: SetupServiceLifecycle
    gateway_verification: SetupGatewayVerification
    state_removal: SetupProductStateRemoval
...
class SetupOperation:
    ...
    def __init__(
        ...
        *,
        ...
        self._nested_operations = nested_operations
    ...
    def _execute_setup_step(
        ...
        if step.id == "supervisor.activate":
            return self._nested_operations.service_lifecycle.activate_supervisor(
                ActivateSupervisor(step_fingerprint=step.fingerprint),
            )
        if step.id == "runtime.install":
            result = self._nested_operations.runtime_supply.install_runtime(
                InstallRuntime(
        ...
        if step.id == "model.install":
            result = self._nested_operations.model_supply.install_model(
                InstallModel(
                    step_fingerprint=step.fingerprint,
            ...
            if selection.trust_grants:
                ...
                self._nested_operations.desired_state.trust_model(
                    TrustModel(
                        step_fingerprint=step.fingerprint,
        ...
        if step.id == "service.configure":
            ...
            return self._nested_operations.desired_state.configure_service(
                ConfigureService(
                    step_fingerprint=step.fingerprint,
                    service=selection.service_name,
                    model_alias=str(selection.model_alias),
                    runtime=str(runtime["installation_id"]),
                    route=str(selection.service_route),
        ...
        if step.id == "gateway.configure":
            ...
            return self._nested_operations.desired_state.configure_gateway(
                ConfigureGateway(
                    step_fingerprint=step.fingerprint,
        ...
        if step.id == "application.install":
            result = self._nested_operations.application_lifecycle.install_applications(
                InstallApplications(
                    step_fingerprint=step.fingerprint,
                    application_targets=selection.application_targets,
                    preserve_outdated_codex=selection.preserve_outdated_codex,
                    offline=resolved.offline,
                ),
            )
        ...
        if step.id == "application-target.configure":
            ...
            for application_target in selection.application_targets:
                ...
                configured[application_target] = (
                    self._nested_operations.application_configuration.configure_application_target(
        ...
        if step.id in {
            ...
            result = self._nested_operations.native_canaries.test_application_target(
        ...
        if step.id == "verify.request":
            result = self._nested_operations.gateway_verification.verify_gateway(
                VerifyGatewayRequest(
                    step_fingerprint=step.fingerprint,
                    endpoint=str(step.inputs["endpoint"]),
                    model=str(step.inputs["model"]),
                    request=str(step.inputs["request"]),
                    dependency_fingerprint=str(step.inputs["dependency_fingerprint"]),
                ),
            )
    ...
    def _execute_removal_step(self, step: MutationStep) -> Mapping[str, object]:
        if step.id in {"service.drain", "service.stop"}:
            ...
            for service in _strings(step.inputs.get("services", ())):
                if step.id == "service.drain":
                    results[service] = (
                        self._nested_operations.service_lifecycle.drain_service(
                            DrainService(
                                step_fingerprint=step.fingerprint,
                                resource=service,
                            ),
                    ...
                    results[service] = (
                        self._nested_operations.service_lifecycle.stop_service(
                            StopService(
                                step_fingerprint=step.fingerprint,
                                resource=service,
                            ),
        ...
        if step.id == "supervisor.unregister":
            return self._nested_operations.service_lifecycle.unregister_supervisor(
                UnregisterSupervisor(step_fingerprint=step.fingerprint),
            )
        if step.id == "application-target.remove":
            ...
            for application_target in _strings(
                ...
                results[application_target] = (
                    self._nested_operations.application_configuration.remove_application_target(
        ...
        if step.id == "application.remove":
            return self._nested_operations.application_lifecycle.remove_applications(
                RemoveApplications(
                    step_fingerprint=step.fingerprint,
                    applications=tuple(_strings(step.inputs.get("applications", ()))),
                ),
            )
        if step.id == "state.remove":
            paths = tuple(_strings(step.inputs.get("paths", ())))
            return self._nested_operations.state_removal.remove_state(
                RemoveState(
                    step_fingerprint=step.fingerprint,
                    paths=paths,
                ),
...
def validated_plan(plan: Mapping[str, object]) -> dict[str, object]:
    unknown = set(plan) - {
        "application_targets",
        "id",
        "kind",
...
def conservative_setup_outcome(*, malformed: bool) -> Mapping[str, object]:
    return {
...
def target_readiness(outcome: Mapping[str, object]) -> Mapping[str, str]:
...
def combined_readiness(target_readiness: Mapping[str, str]) -> Readiness:
    values = set(target_readiness.values())
...
def application_target_health_issue(
    target: str,
    state: object,
...
def application_target_observation_issue(target: str) -> Mapping[str, object]:
    return {
...
def durable_setup_outcome(
    ...
    if target_readiness:
        readiness = combined_readiness(target_readiness)
    else:
        verification = next(
            (
...
def evidence_kind(scope: str) -> str:
