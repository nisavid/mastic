...
def _summary(name: str) -> str:
    return {
        ...
        "check": "Inspect recorded Supervisor, Gateway, and service health without starting them.",
        "doctor": "Diagnose configuration, lifecycle, routing, and service failures.",
        "logs": "Read bounded mastic logs, optionally for one resource.",
...
def _parameters(name: str) -> tuple[Parameter, ...]:
    resource_help = {
        "runtime": "Runtime Definition or Installation ID; discover values with `mastic runtime available` or `runtime list`.",
    ...
    if name == "model.rollback":
        return (
            ...
            _option(
                "target",
                "Previously retained Model Installation ID from `mastic model list`.",
                required=True,
            ),
        )
    if name == "model.trust":
    ...
    if name.startswith("model.cache.") and name not in {
        ...
        return (
            _argument(
                "resource",
                "Cached Revision identity; discover values with `mastic model cache list`.",
            ),
        )
    if (
    ...
    if name == "config.restore":
        return (
            _argument(
                "revision", "Configuration revision from `mastic config history`."
            ),
        )
    if name == "config.diff":
