...
| `mastic app` | Inspects and explicitly upgrades externally owned applications without changing their owner. |
...
| Command | Behavior |
| --- | --- |
| `mastic app inspect codex` | Discovers the active Vite+-managed Codex installation, resolves the selected npm channel's current release, and reports whether an owner-native upgrade is available without mutating the host. |
| `mastic app upgrade codex` | Prepares and retains exact release and artifact evidence, asks the authenticated local user to confirm it, and delegates only the authorized retained identity to `masticd`. |
...
application configuration and directs the user to review `mastic app
