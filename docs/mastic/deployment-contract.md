...
| Codex model catalog and ownership | `~/.local/state/mastic/clients/` | `MASTIC_STATE_DIR` |
| Runtime Installations | `~/.local/share/mastic/runtimes/` | `MASTIC_DATA_DIR` |
| Logs | `~/Library/Logs/mastic/` | `MASTIC_LOG_DIR` |
| `uv` executable | Discovered from known absolute paths | `MASTIC_UV_EXECUTABLE` |
...
- install or update the mastic package from an immutable systools revision;
...
After configuring Codex, `mastic client inspect codex` must report a healthy
