...
SPEC = importlib.util.spec_from_file_location("mastic_responses_adapter", MODULE_PATH)
...
class ProxyIntegrationTest(unittest.TestCase):
    ...
    def test_auth_rejection_happens_without_upstream_contact(self):
        ...
        status, _, _ = self.request(
            "POST", "/v1/responses", {"input": []}, token="wrong"
        )
        self.assertEqual(401, status)
    ...
    def test_very_long_digit_content_length_is_rejected_without_integer_conversion(
        self,
    ):
        auth = f"Authorization: Bearer {INBOUND_TOKEN}\r\n".encode()
        raw = self.raw_request(
    ...
    def test_optional_upstream_bearer_is_injected_separately(self):
        ...
        self.assertEqual(
            ...
            FakeUpstreamHandler.requests[0]["headers"]["Authorization"],
        )
    ...
    def test_sse_preserves_order_and_unchanged_bytes_and_observes_terminal(self):
        ...
        self.assertLess(
            raw.index(b"response.output_item.added"), raw.index(b"response.completed")
        )
    ...
    def test_sse_heartbeat_precedes_delayed_first_upstream_frame(self):
        ...
        self.assertEqual(200, status)
        heartbeat = b": responses-adapter keep-alive\n\n"
    ...
    def test_plain_json_never_receives_sse_heartbeats(self):
        ...
        self.assertNotIn(b"responses-adapter keep-alive", raw)
    ...
    def test_sse_completed_and_done_finish_downstream_before_upstream_eof(self):
        ...
        try:
            with contextlib.redirect_stderr(stderr):
                ...
                terminal_logged_before_release = (
                    "SSE terminal_completed=true" in stderr.getvalue()
                )
        finally:
    ...
    def test_downstream_disconnect_closes_upstream_stream_and_gateway_stays_healthy(
        self,
    ):
        FakeUpstreamHandler.response_headers = {"Content-Type": "text/event-stream"}
        ...
        client.sendall(request)
        received = b""
        while b": responses-adapter keep-alive\n\n" not in received:
...
class PureTransformAndConfigTest(unittest.TestCase):
    ...
    def test_custom_tool_response_remains_identity_even_with_matching_function_map(
        self,
    ):
    ...
    def test_configuration_requires_explicit_upstream_and_fresh_token(self):
        with self.assertRaises(proxy.ConfigurationError):
            proxy.ProxyConfig.from_env(
                {"MASTIC_RESPONSES_ADAPTER_INBOUND_TOKEN": INBOUND_TOKEN}
            )
        with self.assertRaises(proxy.ConfigurationError):
            proxy.ProxyConfig.from_env(
                {"MASTIC_RESPONSES_ADAPTER_UPSTREAM": "http://127.0.0.1/v1"}
            )
        with self.assertRaises(proxy.ConfigurationError):
            proxy.ProxyConfig(
    ...
    def test_sse_heartbeat_configuration_has_positive_finite_env_contract(self):
        ...
        self.assertEqual(
            15.0, proxy.ProxyConfig.from_env(base_env).sse_heartbeat_seconds
        )
        configured = proxy.ProxyConfig.from_env(
            {**base_env, "MASTIC_RESPONSES_ADAPTER_SSE_HEARTBEAT": "7.5"}
        )
        self.assertEqual(7.5, configured.sse_heartbeat_seconds)
        for invalid in ("0", "-1", "nan", "inf"):
            with (
                self.subTest(invalid=invalid),
                self.assertRaises(proxy.ConfigurationError),
            ):
                proxy.ProxyConfig.from_env(
                    {**base_env, "MASTIC_RESPONSES_ADAPTER_SSE_HEARTBEAT": invalid}
                )
